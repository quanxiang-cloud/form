...
func (t *permit) Export(ctx context.Context, result *Result, opts *ExportOption) error {
	url := fmt.Sprintf(exportPermitURL, formHost, opts.AppID)

	return exportPermitFunc(ctx, url,
		ExportReq{
			AppID: opts.AppID,
			Page:  startPage,
			Size:  maxSize,
...
func (t *permit) Import(ctx context.Context, result *Result, opts *ImportOption) error {
	var (
		index int
		req   ImportReq
		url   = fmt.Sprintf(importPermitURL, formHost, opts.AppID)
	)
...
func exportPermitFunc(ctx context.Context, url string, req ExportReq, result *Result, opts *ExportOption) error {
	totalPage := 0
	...
	for {
		...
		result.Permits = append(result.Permits, resp.Permits...)
